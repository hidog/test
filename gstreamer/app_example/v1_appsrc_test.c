 ...
 * (enough-data signal).
 */
gboolean push_buffer( AppData* app )
{
    gpointer raw_buffer;
    ...
    if (ret != GST_FLOW_OK) /* some error, stop sending data */  
        return FALSE;
...
void start_feed( GstElement* pipeline, guint size, AppData* app )
{
    if( app->source_id == 0 ) 
    {
...
gboolean on_pipeline_message( GstBus* bus, GstMessage* message, AppData* app )
{
...
int main( int argc, char *argv[] )
{
    ...
    app->pipeline = gst_parse_launch( "appsrc is-live=true name=source caps=audio/x-raw,format=S16LE,rate=8000,channels=1,layout=interleaved" 
                                      "! flacenc ! oggmux ! filesink location=flac-audio.ogg", NULL );
