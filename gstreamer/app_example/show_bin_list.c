...
void find_interface_elements( GstElement *pipeline )
{
    GstBin *bin;
    gboolean done;
    GstIterator *it;
    GValue data = { 0, };
    bin = GST_BIN_CAST (pipeline);
    ...
    while (!done) 
    {
        switch (gst_iterator_next (it, &data)) 
        {
            case GST_ITERATOR_OK:
            {
                ...
                GstElement *elm = gst_bin_get_by_name( pipeline, GST_ELEMENT_NAME(child) );
                //int elm_type = G_VALUE_TYPE(elm);
                gst_element_print_properties( elm );
                printf("\n\n");
...
int main( int argc, char *argv[] ) 
{
    ...
    
    {
        ...
        printf("list element : \n");
        find_interface_elements(pipeline);
        printf("finish \n\n");*/
    }
    
    {
        GstElement *pipeline = gst_parse_launch ("playbin uri=http://docs.gstreamer.com/media/sintel_trailer-480p.webm", NULL);
        find_interface_elements(pipeline);
    }
