...
{ 
    // 底下的code會報錯
    /*guint size = 0;
    GParamSpec **spec = g_object_class_list_properties( src, &size );

    int i = 0;
    for( i = 0; i < size; i++ )
    {
        printf( "spec[%d] name = %s\n", i, spec[i]->name );
    }
   */
    ...
    {
        GstElement *src = gst_element_factory_make ("tcpserversrc", "src");
    

        gst_element_print_properties(src);    
        printf("\n\n");
    }
...

    {
        ...
        gst_element_print_properties(conv);    
        printf("\n\n");
    }  
    
    {
        GstElement *uribin = gst_element_factory_make ("uridecodebin", "uribin");
        gst_element_print_properties(uribin);    
        printf("\n\n");
    }  
