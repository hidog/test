...
{ 
    ...
    char file_path[100];
    static int index = 0;
    sprintf( file_path, "/home/hidog/code/test/gstreamer/jpg_test/jpg/file_%d.jpg", index );
    printf( "index = %d\n", index );
    index++;
...
int main( int argc, char *argv[] )
{
    ...
    //g_object_set( G_OBJECT(data.src), "do-timestamp", TRUE, NULL );
...
#ifdef USE_CPU
    gst_bin_add_many( GST_BIN(data.pipeline), data.src, data.queue_1, data.dec, data.conv, data.queue_2, data.enc, data.mux, data.sink, NULL );
    gst_element_link_many( data.src, data.queue_1, data.dec, data.conv, data.queue_2, data.enc, data.mux, data.sink, NULL );
#elif defined(USE_GPU)
