/*
看起來能寫入檔案,但改寫成jpeg版本的時候跳出錯誤
這個檔案沒測試過,也沒成功過,當參考用
*/

// https://github.com/XunChangqing/gstreamer-appsrc-x264enc-appsink-sample/blob/master/x264encsample.cpp
...
static int width = 0;
static int height = 0;
static char *outfile = "/home/hidog/code/test/gstreamer/jpg_test/test.mp4";
...
static bool bGstreamerThreadStarted = false;
static bool bGstreamerThreadFinished = false;
...
void *GstreamerThread( void *pThreadParam ) 
{
    ...
    char *strPipeline = malloc(8192); //new char[8192];
    ...
    cap_appsrc_to_x264enc = gst_caps_new_simple( "video/x-raw", 
                                                 "format", G_TYPE_STRING, "GRAY8",
                                                 "width", G_TYPE_INT, width, 
                                                 "height", G_TYPE_INT, height, 
                                                 "framerate", GST_TYPE_FRACTION, 24000, 1001, 
                                                 NULL );

    GstCaps *cap_x264enc_to_sink;
    cap_x264enc_to_sink = gst_caps_new_simple( "video/x-h264", 
    ...
    g_object_set( G_OBJECT(appsrc), "caps",
                                    gst_caps_new_simple( "video/x-raw", 
                                                         ...
                                                         "width", G_TYPE_INT, width,
                                                         "height", G_TYPE_INT, height, 
                                                         "framerate", GST_TYPE_FRACTION, 24000, 1001, NULL ), NULL );
...
char *PullBuffer( int *outlen ) 
{
    ...
    char *pRet = malloc(map.size); //new char[map.size];
    // Copy image
...
int main( int argc, char *argv[] ) 
{
    ...
    width = 1280; //atoi( argv[1] );
    height = 720; //atoi( argv[2] );
    //outfile = argv[3];

    // Validate command line arguments
    if( width < 100 || width > 4096 || height < 100 || height > 4096 ) 
    {
        fprintf( stderr, "width and/or height or quality is bad, not running conversion\n" );
        return -1;
    }
